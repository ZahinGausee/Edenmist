...
export default function CheckoutPageClient({ initialAddress, initialProduct }) {
  ...
  useEffect(() => {
    if (window.Razorpay && !razorpayInstance.current) {
      razorpayInstance.current = window.Razorpay;
    }
  }, []);
  ...
  const createOrder = async () => {
    ...
    try {
      ...
      if (razorpayInstance.current) {
        const payment = new razorpayInstance.current(paymentData);
      ...
      <div className="container mx-auto p-4 max-w-6xl">
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <div>
              ...
              <Button className="w-full mt-6 bg-orange-500 hover:bg-orange-600 text-white py-6" onClick={createOrder}>
                Place Order
              </Button>
            </div>
          </div>
